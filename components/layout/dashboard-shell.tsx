...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const { title, breadcrumb } = useMemo(() => {
    ...
    const labelSegments = segments.map(humanize);

    return {
      title: labelSegments.at(-1) ?? "Dashboard",
      breadcrumb: ["Dashboard", ...labelSegments].join(" / ")
    };
  }, [pathname]);

  return (
    <div className="flex min-h-[calc(100vh-48px)] gap-4">
      ...
      <div className="flex-1 space-y-4 pb-16">
        <div className="sticky top-4 z-30 flex items-center justify-between rounded-2xl border border-white/10 bg-card/60 px-4 py-3 backdrop-blur-xl">
          <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="rounded-lg border border-white/10 bg-white/5 p-2 text-gray-300" aria-label="Notifications">
              <Bell size={16} />
            </button>
